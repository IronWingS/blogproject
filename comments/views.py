...
def post_comment(request,post_pk):
    ...
    # HTTP请求有get和post两种，一般用户通过提交表单提交数据都是通过post请求，
    ...
    if request.method == 'POST':
        ...
        # 当调用form.is_valid()方法是，django自动帮我们检查表单的数据是否符合格式要求。
        if form.is_valid():
            # 检查到数据是合法的，调用表单的save方法保存数据到数据库，
            # commit=False的作用仅仅是利用表单的数据生成Comment模型类的实例，但还不保存评论数据到数据库。
            ...
            # 重定向到post详情页，实际上当redirect函数接受一个模型的实例时，它会调用这个模型实例的
            # get_absolute_url方法
